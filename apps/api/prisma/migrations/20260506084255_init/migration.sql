...
CREATE TABLE `Payment` (
    `id` INTEGER NOT NULL AUTO_INCREMENT,
    `orderId` INTEGER NOT NULL,
    `provider` VARCHAR(191) NOT NULL DEFAULT 'paytr',
    `merchantOid` VARCHAR(64) NOT NULL,
    `providerToken` VARCHAR(191) NULL,
    `status` ENUM('INIT', 'PENDING', 'SUCCESS', 'FAILED') NOT NULL DEFAULT 'INIT',
    `amount` DECIMAL(10, 2) NOT NULL,
    `callbackPayloadJson` JSON NULL,
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updatedAt` DATETIME(3) NOT NULL,

    UNIQUE INDEX `Payment_orderId_key`(`orderId`),
    UNIQUE INDEX `Payment_merchantOid_key`(`merchantOid`),
    INDEX `Payment_merchantOid_idx`(`merchantOid`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
